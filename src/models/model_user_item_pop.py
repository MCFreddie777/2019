...
class ModelUserItemPopularity():
    """
    Model based on total number of clicks per item on unique users.
    """
    ...
    def fit(self, df):
        """Count the number of clicks of unique users for each item."""
    ...
    def predict(self, df):
        """Sort the impression list by number of clicks of unique users in the training phase."""
        ...
        df_rec = (
            df_impressions
            .sort_values(by=["user_id", "session_id", "timestamp", "step", 'n_users'], ascending=[True, True, True, True, False])
